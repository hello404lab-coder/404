...
import { siteContent, type CapabilityContent } from '../data/siteContent'
...
function BentoCard({
  ...
  return (
    ...
    >
      {card.mockupKey ? MOCKUPS[card.mockupKey] : null}
...
export function ProactiveCoreSection() {
  ...
  return (
    <section className="relative z-1 mt-20 px-[70px] max-[1180px]:px-7 max-[720px]:px-[18px] pb-20">
      ...
      >
        <SparkleIcon />
        <span className="text-[rgba(200,170,255,0.9)] text-[0.78rem] font-medium tracking-[-0.01em]">{CAPABILITIES.badge}</span>
      </motion.div>
      ...
      <div className="grid grid-cols-[1fr_1.42fr] gap-4 items-stretch max-[720px]:grid-cols-1">
        {CAPABILITIES.items.map((card, index) => (
          <BentoCard
            key={card.id}
            card={card}
            index={index}
